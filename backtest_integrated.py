"""
Mathayo 154 Model - Integrated Backtest (v3.4)
Preserves your original walk-forward validation and feature set while seamlessly
integrating historical Understat rolling xG data sources (leakage-free).
"""
import pandas as pd
import numpy as np
...
# 2. Merge historical Understat xG rolling features if available
try:
    xg_df = pd.read_csv('understat_xg_rolling_features.csv')
    xg_df['date'] = pd.to_datetime(xg_df['date'])
    
    # Merge on MatchDate/date, HomeTeam, AwayTeam
    df = pd.merge(
        df, xg_df[['date', 'home_team', 'away_team', 'home_xg_form5', 'home_xga_form5', 'away_xg_form5', 'away_xga_form5']],
        left_on=['MatchDate', 'HomeTeam', 'AwayTeam'],
        right_on=['date', 'home_team', 'away_team'],
        how='left'
    )
    # Fill missing historical xG gracefully if a match isn't in Understat top-5 database
    xg_cols = ['home_xg_form5', 'home_xga_form5', 'away_xg_form5', 'away_xga_form5']
    for col in xg_cols:
        if col not in df.columns:
            df[col] = 1.35 # league average baseline fallback
        else:
            df[col] = df[col].fillna(df[col].mean())
    print("Successfully merged Understat historical xG rolling features.")
except Exception as e:
    print(f"Notice: Understat rolling features CSV not found/skipped ({e}). Proceeding with standard features.")
    for col in ['home_xg_form5', 'home_xga_form5', 'away_xg_form5', 'away_xga_form5']:
        df[col] = 1.35

# 3. Compute Pi-ratings walk-forward
...
df['XgForDiff'] = df['home_xg_form5'] - df['away_xg_form5']
df['XgDefDiff'] = df['home_xga_form5'] - df['away_xga_form5']
...
feature_cols = ['EloDiff', 'EloTotal', 'Form5Diff', 'Form3Diff',
                 ...
                 'home_xg_form5', 'home_xga_form5', 'away_xg_form5', 'away_xga_form5',
                 'XgForDiff', 'XgDefDiff']

X = df[feature_cols].values
y = df['FTResult'].values

# 5. Walk-Forward Backtest Validation
n = len(df)
...
print()
print(f"{'Metric':<20} {'v3: +xG + Pi-ratings':<22} {'Market Favorite':<18}")
print(f"{'Accuracy':<20} {model_acc*100:.2f}%{'':<15} {market_acc*100:.2f}%")
print(f"{'Log Loss':<20} {model_logloss:.4f}{'':<16} {market_logloss:.4f}")
print(f"{'RPS':<20} {model_rps:.4f}{'':<16} {market_rps:.4f}")
print()
if importance_pairs:
